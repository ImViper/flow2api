...
服务启动后,访问管理后台: **http://127.0.0.1:51075**,首次登录后请立即修改密码!

当前这台机器已经按局域网调用方式启动:

- 本机访问: `http://127.0.0.1:51075`
- 局域网访问: `http://192.168.10.153:51075`
- OpenAI 兼容 Base URL: `http://192.168.10.153:51075/v1`
- Gemini 兼容 Base URL: `http://192.168.10.153:51075`

给其他人调用时，请使用 `http://192.168.10.153:51075`，不要使用 `127.0.0.1` 或 `localhost`。如果其他机器打不开，先检查 Windows 防火墙是否放行 TCP `51075`。
...
访问 **http://127.0.0.1:51075/test** 可打开内置的模型测试页面，支持：
...
```bash
curl -X POST "http://192.168.10.153:51075/models/gemini-3.1-flash-image:generateContent" \
...
```bash
curl -X POST "http://192.168.10.153:51075/v1/chat/completions" \
